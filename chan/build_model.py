...
DATABASE_COLUMNS = ["num", "thread_num", "op", "timestamp", "comment"]
...
class Chanalysis:
    ...
    def load_archive(self):
        ...
        with sqlite3.Connection(self.database) as conn:
            ...
            conn.executemany(insert_sql, yield_line(filename, self._parse_for_database))
    ...
    def extract_meta(self):
        ...
        with open(filename, "wt") as f:
            ...
            for df in yield_line(self.board, self._parse_for_meta):
                ...
                df["landscape"] = 1 if int(df["media_w"]) > int(df["media_h"]) else 0
    ...
    def build_phrases(self):
        threads = ReadThreads(
            self.board, self.input_dir, return_func=lambda x, y: (x, y.split())
        )
        filename = op.join(self.input_dir, f"{self.board}.trigrams")
        ...
        with open(filename, "wt") as f:
            for num, thread in threads:
                line = " ".join(
                    [
                        word
                        for word in trigram_mod[thread]
